...
install_trojan_panel() {
  if [[ -z $(docker ps -a -q -f "name=^trojan-panel$") ]]; then
    ...
    docker pull jonssonyan/trojan-panel:2.1.0 &&
...
EOF
    ...
    docker pull jonssonyan/trojan-panel-ui:2.1.0 &&
      docker run -d --name trojan-panel-ui --restart always \
        --network=host \
        -v "${UI_NGINX_CONFIG}":"/etc/nginx/conf.d/default.conf" \
        -v ${CERT_PATH}:${CERT_PATH} \
        jonssonyan/trojan-panel-ui:2.1.0
...
update_trojan_panel() {
  ...
  if [[ "${trojan_panel_current_version}" != "${trojan_panel_latest_version}" ]]; then
    ...
    docker rm -f trojan-panel &&
      docker rmi -f jonssonyan/trojan-panel:2.1.0
    ...
    docker rm -f trojan-panel-ui &&
      docker rmi -f jonssonyan/trojan-panel-ui:2.1.0
...
main() {
  ...
  echo_content skyBlue "Version: v2.1.0"
